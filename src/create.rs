...
struct WeatherStation(pub &'static str, pub f32);

impl WeatherStation {
    pub fn measurement(&self) -> f32 {
        let normal = Normal::new(self.1, 10.0).unwrap();
        let m = normal.sample(&mut rand::thread_rng());
        (m * 10.0).round() / 10.0
    }
}

static CITIES: &'static [WeatherStation] = &[
...
fn main() {
    if Path::new(MEASUREMENT_FILE).exists() {
        remove_file(MEASUREMENT_FILE).unwrap();
    }

    File::create(MEASUREMENT_FILE).unwrap();
    ...
    for _i in 0..100 {
        ...
        handles.push(thread::spawn(move || {
            ...
            for _n in 0..10_000_000 {
                ...
                let station: &WeatherStation = &CITIES[idx];
                s.push_str(station.0);
                s.push_str(";");
                s.push_str(&format!("{:.1}", station.measurement()));
                s.push_str("\n");
            }

            let mut writer = BufWriter::with_capacity(204800, file);
    ...
    loop {
        ...
        if handles.iter().all(|x| x.is_finished()) {
            break;
        }
    }
}
